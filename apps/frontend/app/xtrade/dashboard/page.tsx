...
export default function Dashboard() {
  ...
  const [balanceAmount, setBalanceAmount] = useState<AxiosResponse | string>();

  async function BalanceCheck() {
    axios.defaults.withCredentials = true;
    const balance = await axios.get("http://localhost:3001/api/v1/balance");
    if (!balance) {
      return;
    }
    const newBalance = balance.data
    setBalanceAmount(parseFloat(newBalance.balance).toFixed(2));
  }
        ...
        <div className="bg-gray-900 rounded-xl p-6 shadow-lg flex flex-col space-y-6">
          ...
          <div className="flex justify-between">
            ...
            <button
              ...
              className="cursor-pointer w-40 py-2 bg-red-500 hover:bg-red-600 rounded-md text-xl font-bold transition"
            >
              SELL
            </button>
          </div>
